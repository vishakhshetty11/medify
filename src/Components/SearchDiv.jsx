  ...
  const getCity = async (States) => {
    try {
      const resp = await fetch(
        `https://meddata-backend.onrender.com/cities/${States}`
      );
      if (!resp.ok) throw new Error("Network response was not ok");
      const data = await resp.json();
      setCity(data);
  ...
  return (
    <div id={styles.searchDiv}>
      ...
      <div
        ...
        onClick={() => setOpenStateDropdown(true)}
      >
        <TextField
          select
          value={selectedState}
          open={openStateDropdown}
          onOpen={() => setOpenStateDropdown(true)}
          onClose={() => setOpenStateDropdown(false)}
          onChange={(e) => {
      ...
      <div
        ...
        onClick={() => {
          if (selectedState) setOpenCityDropdown(true);
        }}
      >
        <TextField
          select
          ...
          onOpen={() => setOpenCityDropdown(true)}
          onClose={() => setOpenCityDropdown(false)}
          onChange={(e) => {
